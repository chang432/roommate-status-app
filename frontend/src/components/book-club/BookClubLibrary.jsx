...
function bookDate(book) {
  if (book.status === "active") return "Currently reading";
    ...
    <div className={styles.detail}>
      ...
      <form className={styles.reviewForm} onSubmit={submitReview}>
        <div className={styles.formHeading}>
          <div>
            <h3>Your review</h3>
            {initialReview?.finished == null && initialReview && (
        ...
        <fieldset>
          ...
          <div className={styles.finishChoices}>
            <label>
              <input type="radio" name={`book-finished-${book.id}`} value="true" checked={finished === "true"} onChange={(event) => setFinished(event.target.value)} required disabled={busy} />
              <span>Finished</span>
            </label>
            <label>
              <input type="radio" name={`book-finished-${book.id}`} value="false" checked={finished === "false"} onChange={(event) => setFinished(event.target.value)} required disabled={busy} />
        ...
        <button className="ui-primaryButton" type="submit" disabled={busy || !rating || !finished}>
          {busy ? "Saving…" : initialReview ? "Update review" : "Save review"}
        </button>
      </form>

      <section className={styles.community} aria-labelledby={`community-reviews-${book.id}`}>
        <div className={styles.sectionHeading}>
          <div>
            <p className={styles.eyebrow}>From the household</p>
            <h3 id={`community-reviews-${book.id}`}>Community reviews</h3>
          </div>
      ...
      <section className={styles.discussions} aria-labelledby={`book-discussions-${book.id}`}>
        <div className={styles.sectionHeading}>
          <div>
            <p className={styles.eyebrow}>Across every meeting</p>
            <h3 id={`book-discussions-${book.id}`}>Discussions</h3>
          </div>
          <span>{book.meetings.length} {book.meetings.length === 1 ? "meeting" : "meetings"}</span>
        </div>
        ...
        <div className={styles.meetingForums}>
          {book.meetings.map((meeting) => (
            <article className={styles.meetingForum} key={meeting.id}>
              <header>
                <div>
                  <p className={styles.eyebrow}>{meeting.status === "scheduled" ? "Open meeting" : "Completed meeting"}</p>
                  <h4>{exactDateTime(meeting.scheduledAt)}</h4>
                </div>
                <p>{meeting.readingTarget}</p>
              </header>
              <BookClubForum
                meeting={meeting}
  ...
  if (selectedBookId && !selected) {
    return (
      <div className={styles.empty}>
        ...
        <p>This title is no longer available in this household.</p>
        <button type="button" className="ui-primaryButton" onClick={onBack}>View all books</button>
      </div>
    );
  }
  ...
  return (
    <section className={styles.library} aria-label="Book library">
      <div className={styles.libraryHeading}>
        <div>
          <p className={styles.eyebrow}>Active and completed</p>
          <h2>{books.length} {books.length === 1 ? "book" : "books"}</h2>
        </div>
      ...
      {!books.length ? (
        <div className={styles.empty}>
          <span aria-hidden="true">📚</span>
          <h2>No books yet</h2>
          <p>The current book and completed titles will collect here.</p>
        </div>
      ) : (
        <div className={styles.bookList}>
          {filteredBooks.map((book) => (
            <button key={book.id} type="button" onClick={() => onSelectBook(book.id)} className={styles.bookCard}>
              <span className={styles.bookSpine} aria-hidden="true" />
              <span className={styles.bookCopy}>
                <span className={styles.cardTopline}>
                  <small>{bookDate(book)}</small>
                  {book.status === "active" && <em>Active</em>}
                </span>
                <strong>{book.title}</strong>
                <span>by {book.author}</span>
                <small>Owned by {book.bookOwnerName || "Former member"}</small>
                <span className={styles.cardStats}>
                  <span>{ratingLabel(book)}</span>
                  <span>{finishedLabel(book)}</span>
                </span>
              </span>
